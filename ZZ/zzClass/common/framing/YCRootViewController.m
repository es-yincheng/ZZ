...
@interface YCRootViewController ()<UIGestureRecognizerDelegate,UITableViewDelegate>{
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        _coverButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, SCRENW, SCRENH)];
...
- (void)viewDidLoad {
    ...
    _leftViewController.tableView.delegate = self;
}
...
{
    YCLog(@"I did selected %ld item in rootview!",(long)indexPath.row);
...
//    NSNotification *notification = [NSNotification notificationWithName:@"" object:nil];
    //2.注册通知  在 homeViewController 中注册监听
    //3.发送通知
    
    Class c = NSClassFromString([self.leftViewController.cellControllerArray objectAtIndex:indexPath.row]);
    UIViewController *viewctroller = [[c alloc] init];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"jmpfromhomevc" object:viewctroller];
}


- (void)setRootViewController:(UITabBarController *)rootViewController{
...
-(void)setLeftViewController:(YCLeftViewController *)leftViewController{
...
//        [_currentView setFrame:CGRectMake(100, 80, SCRENW, SCRENH-160)];
    }else{//向左滑的
