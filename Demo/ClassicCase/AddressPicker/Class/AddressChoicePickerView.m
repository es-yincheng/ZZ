...
- (instancetype)init{
    ...
    if (self = [super init]) {
        ...
        NSMutableArray *tempProvinceArr = [[NSMutableArray alloc] init];
        for (NSDictionary *dict in self.regionArr) {
            NSArray *tempPro = dict[@"provinces"];
            for (NSDictionary *prodict in tempPro) {
                [tempProvinceArr addObject:prodict];
//                NSLog(@"%@",prodict[@"province"]);
            }
        }
        self.provinceArr = [tempProvinceArr copy];
//        NSLog(@"%ld",(long)self.provinceArr.count);
